# Moto Seat Lab

Concept-stage content project for a future multilingual website about motorcycle seat comfort, DIY seat modification, ready-made seat options, materials, tools, and rider-specific ergonomics.

## Current Phase

**Phase 0: Concept and content architecture**

No website code yet. The current goal is to define:

- target audience,
- content structure,
- first country and language scope,
- first motorcycle model guide,
- research backlog,
- affiliate/monetization strategy,
- roadmap.

## Project Mission

Help motorcycle riders understand why their seat hurts, what can be improved, what can be bought, what can be built, and how to choose a solution based on their motorcycle, body, budget, ride duration, climate, and DIY skill level.

## First Scope

Pilot scope:

- Country: Germany
- Primary language: German
- Working source language: Slovak
- First motorcycle: Suzuki GSX-S1000GX
- First rider problem: discomfort after 1 hour of riding

## Target Audience

- DIY riders who want to modify their own seat.
- Riders who are uncomfortable after short or medium rides.
- Touring riders who need comfort for longer days.
- Owners of specific motorcycle models looking for model-specific advice.
- People comparing ready-made seats, gel pads, 3D mesh covers, upholstery work, or full DIY builds.

## What The Future Website Should Include

- Motorcycle seat theory.
- Motorcycle type guides.
- Use-case guides for short, long, city, touring, off-road, and passenger riding.
- Bike-specific guides.
- Country-specific buying guides.
- Material and tool comparisons.
- DIY checklists.
- Ergonomic measurement guides.
- Affiliate product recommendations.
- Maintenance and care guides.

## Documentation Map

- [Project status](docs/STATUS.md)
- [Concept](docs/concept.md)
- [Audience](docs/audience.md)
- [Content architecture](docs/content-architecture.md)
- [Design system concept](docs/design-system-concept.md)
- [Roadmap](docs/roadmap.md)
- [Interactive tools concept](docs/interactive-tools.md)
- [Beginner to expert funnel](docs/beginner-to-expert-funnel.md)
- [Data maintenance tool](docs/data-maintenance-tool.md)
- [News, community, source directory, and translation concept](docs/news-community-translation-concept.md)
- [Data entry templates](docs/data-entry-templates.md)
- [Maintenance workflow](docs/maintenance-workflow.md)
- [Validation and import rules](docs/validation-and-import-rules.md)
- [Data model concept](docs/data-model.md)
- [Calculation model concept](docs/calculation-model.md)
- [Calculation input skeleton](docs/calculation-input-skeleton.md)
- [Technical parameter model](docs/technical-parameter-model.md)
- [Seat comparison concept](docs/seat-comparison.md)
- [Feature roadmap](docs/feature-roadmap.md)
- [Implementation notes](docs/implementation-notes.md)
- [Country and localization strategy](docs/countries-and-localization.md)
- [Bike database strategy](docs/bike-database.md)
- [Suzuki GSX-S1000GX first guide](docs/bike-guides/suzuki-gsx-s1000gx.md)
- [Germany first country guide](docs/country-guides/germany.md)
- [DIY seat requirements](docs/content/diy-seat-requirements.md)
- [Affiliate strategy](docs/monetization/affiliate-strategy.md)
- [Research backlog](docs/research/research-backlog.md)
- [Competitive landscape](docs/research/competitive-landscape.md)
- [First German page outline](docs/content/first-german-page-outline.md)
- [Solution decision tree](docs/content/solution-decision-tree.md)
- [SEO and taxonomy](docs/content/seo-and-taxonomy.md)

## Working Principle

Start with structured content before building the website.

The first deliverable should be a high-quality German landing/content page for Suzuki GSX-S1000GX seat comfort, backed by a reusable content model for more motorcycles and countries.
